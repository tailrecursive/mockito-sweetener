...
trait MockitoScalaTestMatcherAdapters {
  ...
  implicit def should[T : ClassTag](matcher: Matcher[T]): T = M.argThat[T](new WrappedScalaTestMatcher(matcher))
  ...
  def within[T : ClassTag](spread: Spread[T]): T = M.argThat[T](new SpreadMatcher(spread))
}
